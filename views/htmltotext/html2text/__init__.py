...
from views.htmltotext.html2text.compat import urlparse, HTMLParser
from views.htmltotext.html2text import config

from views.htmltotext.html2text.utils import (
    name2cp,
    unifiable_n,
...
class HTML2Text(HTMLParser.HTMLParser):
    def __init__(self, out=None, baseurl='', bodywidth=config.BODY_WIDTH):
        ...
        self.ul_item_mark = '*ul'  # covered in cli
    ...
    def outtextf(self, s):
        ...
        # crear un dict 
        token = s.split(" ")
        # si el titulo coincide con la url es el comienzo
        # hay que calcular la probabilidad segun la oracion tenga keys
        # similares a la oracion siguiente o a las anteriores
        # si son oraciones muy cortas y sin relacion, se escluye
        #print token
        ...
        #
        #
        #print [len(s.decode("utf-8").split(" ")), " " + s]
        #print [s]
        #print str(str(self.start) + "  " + self.current_tag) + " " + str(len(self.preceding_data.split(" "))) + " ::ss" + str([s])
        #print "outtextlist:  "
        #print [len(self.outtextlist)]

        
        #sys.exit()
        
        if "#BORRAR" in s and len(self.outtextlist) > 5: self.continuar = True
        #<print self.continuar
        
        if str(self.current_tag) in ["a"] and not self.continuar and (len(self.outtextlist) > 5):
            self.outtextlist.append(s)
        if str(self.current_tag) in ["p","b","br","em","strong"]:
            self.outtextlist.append(s)
        if s:
            self.lastWasNL = s[-1] == '\n'
        ...
        if False:
            c = 0
            if s.decode("utf-8") == "\n\n":
                self.outtextlist.append("\n")
            if len(s.decode("utf-8").split(" ")) > 6:#len(token) > 6 or s.isspace():
    ...
    def handle_tag(self, tag, attrs, start):
        ...
        if hn(tag):
            ...
            if start:
                self.inheader = True
                #xxx = self.outtextlist[-1]
                #self.outtextlist = [xxx]
                #print "-------0------"
                
                
                self.o(hn(tag) * "#BORRAR" + ' ')
                #print self.outtextlist
                #time.sleep(100)
            else:
                #print "-------else------"
                self.inheader = False
        ...
        if tag in ["ol", "ul"] and self.manage_ul:
        ...
        if tag == 'li' and self.manage_ul:
            ...
            if start:
                ...
                self.o("  BORRAR " * nest_count)
                if li['name'] == "ul":
                    pass
                    #self.o(self.ul_item_mark + " ")
                elif li['name'] == "ol": pass
                    #li['num'] += 1
                    #self.o(str(li['num']) + ". ")
                self.start = 1
